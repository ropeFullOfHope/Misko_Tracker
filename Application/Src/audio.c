...
static uint16_t audio_buffer[2][BUFFER_SIZE / 2];
...
void audio_start(void)
{
    ...
    for (uint32_t channel = 0; channel < CHANNEL_COUNT; channel++) {
        ...
        channel_data[channel].current_note = 0;
        channel_data[channel].current_volume = 0;
        channel_data[channel].current_instrument_id = 0;
    }
...
static int16_t audio_get_next_sample(uint32_t channel) {
    ...
    if (channel_data[channel].row_sample_index >= samples_per_row) {
        ...
        if (channel_data[channel].phrase_index >= PHRASE_ROW_COUNT)
        {
                ...
                project_data.chain[channel_data[channel].current_chain].phrase[channel_data[channel].chain_index] == 0x00)
            {
                    ...
                    project_data.song.chain[channel][channel_data[channel].chart_index] == 0x00)
                {
                    ...
                    channel_data[channel].current_chain = project_data.song.chain[channel][channel_data[channel].chart_index];
            }
            channel_data[channel].current_phrase = project_data.chain[channel_data[channel].current_chain].phrase[channel_data[channel].chain_index];
        }
        ...
        instrument_id_t new_instrument_id = project_data.phrase[channel_data[channel].current_phrase].instrument[channel_data[channel].phrase_index];
        if (new_instrument_id != 0x00)
